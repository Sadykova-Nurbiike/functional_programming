...
and2 = foldr (&&) True
//Start = and1 [True, True, False] // False
...
g :: [[Int]] -> Int
g [] = 0
g [x:xs] = foldr (+)0 (map (\x = x/x) (flatten[x:xs]))
//g [x:xs] = sum (map length [x:xs])
//g [x:xs] = foldr (+) 0 (map length [x:xs])
